...
Note: T1 schema tests are skipped if T1 code isn't present (separate branch).
"""
...
# Conditional import — T1 may not be present on this branch
try:
...
@pytest.mark.skipif(not HAS_T1, reason="T1 api.schemas not available")
...
class TestFullShoppingFlow:
    ...
    def test_shopping_with_aisle_info(self) -> None:
        """Test that ShoppingItem-style dicts work with the flow."""
        tracker = InventoryTracker()
        tracker.update_available(["pasta", "eggs"])

        # Simulate T1 returning structured shopping items
        shopping_items = [
            {"name": "guanciale", "quantity": "150g", "aisle": "deli"},
            {"name": "black pepper", "quantity": "1 tsp", "aisle": "spices"},
        ]

        # Set needed from shopping items + available
        tracker.set_needed(
            [i["name"] for i in shopping_items] + ["pasta", "eggs"]
        )

        # Verify missing
        assert tracker.get_shopping_list() == ["black pepper", "guanciale"]

        # Robot collects both
        tracker.add_to_cart("guanciale")
        result = tracker.add_to_cart("black pepper")
        assert result.complete is True
        assert "Requirements met" in result.message
...
class TestVisionRoutesWithT1:
    ...
    def test_full_api_shopping_flow(self) -> None:
        ...
        # Get shopping list (all missing since no scan)
